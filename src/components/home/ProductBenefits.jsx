      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        {benefits.map((benefit, i) => (
          <div key={i} className={`p-12 rounded-[3.5rem] border transition-all duration-700 ${i === 1 ? 'bg-primary border-primary shadow-2xl shadow-red-200' : 'bg-slate-50 border-slate-100 hover:bg-white hover:shadow-xl'}`}>
            <div className={`w-20 h-20 rounded-3xl flex items-center justify-center text-4xl mb-10 shadow-inner ${i === 1 ? 'bg-white/20 text-white' : 'bg-white shadow-xl text-primary'}`}>
              {i === 0 && <FiWind />}
              {i === 1 && <FiHeart />}
              {i === 2 && <FiCoffee />}
            </div>
            ...
            <ul className="space-y-4">
              {benefit.items.map((item, j) => (
                <li key={j} className="flex items-center gap-4">
                  <div className={`w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-black ${i === 1 ? 'bg-white text-primary' : 'bg-primary text-white'}`}>
                    <FiCheck />
                  </div>
