...
const BlogModal = ({ isOpen, onClose, blog, onSave, loading }) => {
  ...
  const labelCls = 'text-[13px] font-semibold text-slate-400 uppercase tracking-widest pl-1 mb-2 block';
      ...
      <div className="bg-white rounded-[2rem] w-full max-w-4xl shadow-2xl overflow-hidden border border-slate-100 font-sans flex flex-col transform transition-all h-[90vh]">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="space-y-6">
                ...
                >
                  {previewUrl ? (
                    ...
                    <div className="text-center space-y-3">
                      ...
                      <div>
                        <p className="text-[14px] text-slate-800 font-bold uppercase tracking-widest">Pick Cover</p>
                        <p className="text-[12px] text-slate-400 uppercase font-semibold mt-1">JPG, PNG, WebP</p>
                      </div>
                    </div>
                  )}
              ...
              <div className="bg-slate-50 border border-slate-100 rounded-2xl p-6 flex justify-between items-center">
                 <div>
                    <p className="text-[14px] font-black text-slate-800 uppercase tracking-widest">Published</p>
                    <p className="text-[13px] font-bold text-slate-400 uppercase mt-0.5">{formData.status}</p>
                 </div>
                 <button 
                  type="button"
        ...
        <div className="px-8 py-6 bg-slate-50 border-t border-slate-50 flex gap-4 justify-end items-center shrink-0">
          <div className="flex gap-3">
             <button onClick={onClose} className="px-8 py-3 text-slate-500 font-bold text-[14px] uppercase tracking-widest hover:bg-slate-100 rounded-2xl transition">
               Cancel
             </button>
             <button 
              onClick={handleSubmit} 
              disabled={loading}
              className="px-12 py-3 bg-primary text-white font-bold text-[14px] uppercase tracking-widest rounded-2xl shadow-xl shadow-primary/20 hover:scale-105 active:scale-95 transition-all disabled:opacity-50"
             >
