...
>>>>>>> Stashed changes
  ...
  const handleFeatureIconChange = (idx, iconName) => {
                    ...
                    <div key={i} className="flex gap-2 items-center group relative">
                      <div className="relative">
                        <button
                          ...
                          className="w-10 h-10 border border-slate-200 rounded-lg bg-white flex items-center justify-center text-slate-600 hover:border-primary transition"
                        >
                          {ICON_OPTIONS.find(o => o.name === f.icon)?.icon || <FiPackage />}
                        </button>
                        
                        {openIconPicker === i && (
                          <div className="absolute top-full left-0 mt-2 p-3 bg-white border border-slate-200 rounded-xl shadow-2xl z-[100] grid grid-cols-5 gap-2 w-48 animate-fade-in">
                            {ICON_OPTIONS.map(opt => (
                              <button
                                key={opt.name}
                                title={opt.name}
                                onClick={() => handleFeatureIconChange(i, opt.name)}
                                className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-slate-100 text-slate-500 hover:text-primary transition hover:scale-110"
                              >
                                {opt.icon}
                              </button>
                            ))}
                      ...
                      <input type="text" value={f.label} onChange={e => updateFeature(i, 'label', e.target.value)}
                        className="flex-1 text-[13px] border border-slate-200 rounded-lg px-4 py-2 outline-none focus:border-primary placeholder:text-slate-300" placeholder="Feature description..." />
