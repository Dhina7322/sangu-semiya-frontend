...
const RecipeModal = ({ isOpen, onClose, recipe, onSave, loading }) => {
  ...
  const labelCls = 'text-[14px] font-black text-slate-400 uppercase tracking-[0.1em] pl-1 mb-1.5 block';
      ...
      <div className="bg-white rounded-[2rem] w-full max-w-md shadow-2xl overflow-hidden border border-slate-100 font-sans flex flex-col transform transition-all">
        ...
        <div className="px-6 py-4 border-b border-slate-50 flex justify-between items-center shrink-0">
          <div>
            ...
            <p className="text-[13px] text-slate-400 font-semibold uppercase tracking-widest mt-0.5">Recipe sync</p>
          </div>
                ...
                {previewUrl ? (
                   <>
                     ...
                     <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <span className="bg-white text-slate-800 text-[13px] font-black uppercase px-4 py-1.5 rounded-full shadow-lg">Change</span>
                     </div>
                   </>
                ) : (
                   <div className="text-center space-y-1">
                      ...
                      <p className="text-[13px] text-slate-400 font-bold uppercase tracking-widest">Upload</p>
                   </div>
                )}
        ...
        <div className="px-6 py-4 bg-slate-50/50 border-t border-slate-50 flex gap-3 justify-end shrink-0">
          <button type="button" onClick={onClose} className="px-6 py-2 text-slate-500 font-bold text-[13px] uppercase tracking-widest hover:bg-slate-100 rounded-xl transition">
            Close
          </button>
          <button onClick={handleSubmit} disabled={loading}
            className="px-8 py-2 bg-primary text-white font-black text-[13px] uppercase tracking-widest rounded-xl shadow-lg shadow-red-50 hover:scale-105 active:scale-95 transition-all disabled:opacity-50">
